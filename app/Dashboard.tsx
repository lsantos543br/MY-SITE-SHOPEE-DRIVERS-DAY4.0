...
export default function Dashboard({
  ...
  // Alterado para false para iniciar no tema claro por padrao
  ...
  // Quando o filtro de semana está ativo,
  // conta apenas os disparos daquela(s) semana(s)
  // ==========================================
  ...
  // =================================================================
  // PROCESSAMENTO EXCLUSIVO E MAPEAMENTO CRUZA COMPLETO (memoized)
  // =================================================================
  const {
  ...
  return (
    <div className={`p-6 rounded-3xl min-h-screen transition-colors duration-300 ${estilo.fundo}`}>
      ...
      <style>{`
        @media print {
          /* Hide filters and header buttons */
          .print\\:hidden {
            display: none !important;
          }
          /* Remove dark backgrounds, force white bg and black text */
          * {
          ...
          /* Keep colored bar fills and status dots visible */
          ...
          /* Show all tables without scroll/overflow */
          .overflow-x-auto,
          .overflow-y-auto,
          .overflow-hidden {
      ...
      {/* CABECALHO DO DASHBOARD COM BOTAO PDF (Invisivel na impressao) */}
      ...
      <div className="space-y-6">
        {/* FILTROS (Invisivel na impressao) */}
          ...
          <div className={`p-5 rounded-2xl border flex items-center gap-4 ${estilo.card}`}>
            ...
            <div>
              ...
              <h3 className="text-2xl font-black mt-0.5 tracking-tight flex items-baseline gap-2">
                ...
                <span className="text-sm font-bold text-orange-500">/ {disparosEfetivos > 0 ? ((totalLeads / disparosEfetivos) * 100).toFixed(1) : "0.0"}%</span>
              </h3>
            </div>
          </div>
              ...
              <h3 className="text-2xl font-black mt-0.5 tracking-tight flex items-baseline gap-2">
                ...
                <span className="text-sm font-bold text-purple-600 dark:text-purple-400">
                  / {disparosEfetivos > 0
                    ...
                    }).length / disparosEfetivos) * 100).toFixed(1)
                    : "0.0"}%
                </span>
              </h3>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <h3 className="text-2xl font-black mt-0.5 tracking-tight flex items-baseline gap-2">
                ...
                <span className="text-sm font-bold text-blue-500">
                  / {(() => {
                    ...
                    return disparosEfetivos > 0 ? ((compareceram / disparosEfetivos) * 100).toFixed(1) : "0.0";
                  })()}%
                </span>
              </h3>
            </div>
          ...
          <div className={`p-5 rounded-2xl border flex items-center gap-4 ${estilo.card}`}>
            ...
            <div>
              <p className="text-[13px] font-bold uppercase tracking-wider text-[#0046B5] dark:text-blue-700">
                Total First Trip Efetuada / (% conversão sobre total de disparos)
              </p>
              <h3 className="text-2xl font-black mt-0.5 tracking-tight flex items-baseline gap-2">
                <span style={{ color: '#18181b' }}>{totalFirstTrip}</span>
                <span className="text-2xl font-bold text-emerald-500">
                  / {disparosEfetivos > 0 ? ((totalFirstTrip / disparosEfetivos) * 100).toFixed(1) : "0.0"}%
                </span>
              </h3>
            </div>
          </div>

          {/* CARD 6: MEDIA ATENDIMENTOS - DIRETO DA PLANILHA CELULA F6 */}
          <div className={`p-5 rounded-2xl border flex items-center gap-4 ${estilo.card}`}>
            ...
            <div>
              <p className="text-[13px] font-bold uppercase tracking-wider text-[#D44125] dark:text-red-600">
                MEDIA ATENDIMENTOS POR DIA, POR ANALISTA
              </p>
              ...
              <h3 className="text-2xl font-black mt-0.5 tracking-tight flex items-baseline gap-2">
                <span style={{ color: '#18181b' }}>
                  {mediaAtendimentoAnalista > 0
                    ...
                    : (() => {
                        // Fallback: calcula localmente se a planilha nao retornou
                        ...
                        const semanasUnicas = [...new Set(
                          dadosEfetivos.map(m => String(m.weekend || m.Weekend || '').trim()).filter(Boolean)
                        )].length;
                        const totalDias = semanasUnicas > 0 ? semanasUnicas * 7 : 1;
                        const media = totalDias > 0 && TOTAL_ANALISTAS > 0
        ...
        {/* TAXAS DE CONVERSAO ENTRE ETAPAS DO FUNIL */}
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mt-2">
          {(() => {
            ...
            const tNaoCompareceram = dadosEfetivos.filter(m => {
              const st = String(m.status || m.Status || '').trim().toLowerCase();
              return st.includes("nao compareceu") || st.includes("não compareceu");
            }).length;
            ...
            // (calculos antigos removidos - agora tudo dividido por disparos abaixo)

            // FUNIL ACUMULADO: tudo dividido por disparos para os numeros irem descendo
            ...
            const taxaConversaoGeralDisparos = tDisparos > 0 ? ((tFirstTrip / tDisparos) * 100) : 0;
            ...
            const minicards = [
              ...
              { titulo: "Conversao Geral", subtitulo: "First Trip / Disparos", valor: taxaConversaoGeralDisparos, cor: "text-[#D44125]", bordaCor: "border-[#EE4D2D]/30", bgCor: "bg-[#EE4D2D]/5" },
            ];

            // Mapa de cores inline para garantir legibilidade
            ...
            return minicards.map((card, i) => (
              <div key={i} className={`p-4 rounded-2xl border text-center ${estilo.card} ${card.bordaCor} ${card.bgCor}`}>
              ...
              <div className="w-full">
                <div className="flex items-center gap-2 text-orange-500 mb-3">
                  ...
                  <h4 className="text-xs font-black uppercase tracking-wider text-[#FF5722]">
                    TREINADOS X FRIST TRIP POR ANALISTA (CONVERSAO - agendados / first trip)
                  </h4>
                </div>
  ...
  {(() => {
    ...
    return (
      <div className="space-y-4 flex-1 flex flex-col">
        <div className="flex items-center gap-2 text-[#0046B5] dark:text-zinc-400">
        ...
        <div className="flex flex-col items-center justify-center flex-1 pt-2 gap-6">
          ...
          <div className="w-full space-y-2.5">
            <div className="flex items-center justify-between text-xs font-bold border-b border-zinc-500/10 pb-1.5">
              ...
              <span className="font-black" style={{ color: '#18181b' }}>
            ...
            <div className="flex items-center justify-between text-xs font-bold border-b border-zinc-500/10 pb-1.5">
              ...
              <span className="font-black" style={{ color: '#18181b' }}>{tLeads} un</span>
            </div>
            ...
            <div className="flex items-center justify-between text-xs font-bold border-b border-zinc-500/10 pb-1.5">
              ...
              <span className="font-black" style={{ color: '#18181b' }}>{tAgendados} un</span>
            </div>
            ...
            <div className="flex items-center justify-between text-xs font-bold border-b border-zinc-500/10 pb-1.5">
              ...
              <span className="font-black" style={{ color: '#18181b' }}>{tCompareceram} un</span>
            </div>
            ...
            <div className="flex items-center justify-between text-xs font-bold pb-0.5">
              ...
              <span className="font-black" style={{ color: '#18181b' }}>{tFirstTrip} un</span>
            </div>
          </div>
        </div>
      </div>
          ...
          {/* 2: Evolucao de Respostas por Semana */}
          <div className={`p-5 rounded-2xl border h-full flex flex-col justify-between lg:col-span-3 ${estilo.card}`}>
            <div className="space-y-8 flex-1 flex flex-col w-full">
              ...
              <div className="flex items-center gap-2 justify-center w-full border-b pb-3 border-zinc-100 dark:border-zinc-800">
                <TrendingUp className="w-5 h-5 text-[#0046B5] dark:text-blue-400" />
                <h4 className="text-l font-black uppercase tracking-wider text-[#0046B5] dark:text-zinc-400 text-center">
                ...
                <div className="flex items-end justify-center gap-4 sm:gap-6 min-w-max px-4 h70">
                  {semanasOrdenadas.length > 0 ? (
                        ...
                        return (
                          <div key={idx} className="flex flex-col items-center w-16 sm:w-20 justify-end group">
                            <div className="mb-2 text-center flex flex-col items-center justify-end h-10 shrink-0">
                              <span className="text-[15px] font-black text-zinc-100 dark:text-zinc-100 whitespace-nowrap">
                                {sem.total}
                              </span>
                            ...
                            <div className="w-8 sm:w-7.5 h-40 rounded-full bg-zinc-100 dark:bg-blue-300/50 overflow-hidden relative shrink-0 shadow-inner">
                              <div
                            ...
                            <div className="mt-2.5 text-center shrink-0">
                              <span className="text-[15px] font-black font-mono text-zinc-100 dark:text-zinc-400 whitespace-nowrap">
                                {sem.name}
                              </span>
                            </div>
                          </div>
                    ...
                    <div className="flex items-center justify-center h-full w-full py-8">
                      <p className="text-xs text-zinc-100 italic text-center">Nenhuma semana localizada.</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-8">
                {(() => {
                  ...
                  if (!modaisFiltrados || modaisFiltrados.length === 0) {
                    return <p className="text-xs italic text-zinc-500">Nenhum modal encontrado.</p>;
                  }
                    ...
                    return (
                      <div key={i} className="space-y-1">
                        <div className="flex justify-between text-xs font-bold">
                          <span className="uppercase text-[13px] text-zinc-800 dark:text-zinc-200">{modal.name}</span>
                    ...
                    hubsOrdenados.map((hub, i) => {
                      return (
                        <div key={i} className="flex flex-col items-center w-16 sm:w-20 justify-end group">
                          ...
                          <div className="mb-2 text-center flex flex-col items-center justify-end h-10 shrink-0">
                            <span className="text-[15px] font-black text-zinc-900 dark:text-zinc-100 whitespace-nowrap">
                              {hub.qtd}
                            </span>
                          ...
                          <div className={`w-8 sm:w-8 h-36 rounded-full overflow-hidden relative shrink-0 ${estilo.barraFundo} bg-zinc-100 dark:bg-zinc-800/50 shadow-inner`}>
                            <div
                          ...
                          <div className="mt-2.5 text-center shrink-0 w-full max-w-[1000px] truncate">
                            <span className="text-[13px] font-bold text-zinc-600 dark:text-zinc-300" title={hub.name}>
                              {hub.name}
                            </span>
                          </div>
        ...
        <div className="space-y-4 pt-2">
          <div className="flex items-center gap-2 text-zinc-800 dark:text-zinc-200">
            ...
            <h3 className="text-2xl font-bold text-[#0046B5] dark:text-blue-400 uppercase tracking-wider">
              Metricas Customizadas por Modal e Semana
            </h3>
          </div>
                      ...
                      listaTodosModaisEncontrados.map((modalNome, idx) => {
                        ...
                        return (
                          <tr key={idx} className="hover:bg-zinc-50 dark:hover:bg-zinc-800/40 transition-colors">
                            <td className="p-2 font-semibold flex items-center gap-1.5 truncate sticky left-0 bg-white/95 dark:bg-zinc-950/95 text-zinc-900 dark:text-zinc-300 z-20 border-r border-zinc-200 dark:border-zinc-800" title={modalNome}>
                            ...
                            {colunasSemanasUnicas.map((semanaNome, i) => {
                              ...
                              return (
                                <td key={i} className="p-2 text-center font-mono text-zinc-700 dark:text-zinc-300">
                                  {qtdEspecifica > 0 ? (
                                    <span>
